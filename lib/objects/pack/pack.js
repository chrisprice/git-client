...
//      });
      ...
      console.log(i, index.offset, nextIndex.offset, header.headerLength, length, index.hash, header.type, header.typeCode);
      reader.inflate(length, function(error, buffer) {
        if (error) {
          console.log(index.hash, header, length);
          throw error;
        }
...
function readPackedObjectHeader(reader) {
  ...
  var typeCode = (byte0 & parseInt('01110000', 2)) >> 4;
